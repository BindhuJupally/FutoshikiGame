...
class ModeSelectionWindow:
    def __init__(self, root):
        ...
        self.add_events_button()

        # Display the initial message
        self.show_message("Select a game mode to start playing!")

    def add_events_button(self):
        events_button = tk.Button(self.root, text="Events", font=('Arial', 14), command=self.open_events_page, bg='indigo', fg='white', relief=tk.RAISED, bd=5)
        events_button.place(relx=0.9, rely=0.9, anchor=tk.SE, width=100, height=60)

    def open_events_page(self):
        self.root.destroy()
        events_root = tk.Tk()
        EventsWindow(events_root)
        events_root.state('zoomed')
        events_root.mainloop()
...
class EventsWindow:
    def __init__(self, root):
        ...
        # Handle window close event to cancel the timer
        self.root.protocol("WM_DELETE_WINDOW", self.on_closing)

    def add_event_1(self):
        event_frame = tk.Frame(self.root, bg="white", bd=2, relief=tk.SOLID)
        event_frame.place(relx=0.5, rely=0.3, anchor=tk.CENTER, width=600, height=200)

        event_image = Image.open("tn.jpg")
        event_image = event_image.resize((100, 100), Image.LANCZOS)
        self.event_icon = ImageTk.PhotoImage(event_image)
        event_icon_label = tk.Label(event_frame, image=self.event_icon, bg='white')
        event_icon_label.pack(side=tk.LEFT, padx=10, pady=10)

        event_title = tk.Label(event_frame, text="Demon Slayer Tournament 1", font=("Arial", 18, "bold"), bg="white")
        event_title.pack(side=tk.TOP, anchor=tk.W, padx=10, pady=10)

        self.timer_label = tk.Label(event_frame, text="", font=("Arial", 14), bg="white")
        self.timer_label.pack(side=tk.TOP, anchor=tk.W, padx=10, pady=10)
        self.update_timer()

        buttons_frame = tk.Frame(event_frame, bg="white")
        buttons_frame.pack(side=tk.BOTTOM, pady=10)

        play_button = tk.Button(buttons_frame, text="Play", font=('Arial', 14), command=self.play_event, bg='green', fg='white')
        play_button.pack(side=tk.LEFT, padx=10)

        back_button = tk.Button(buttons_frame, text="Back", font=('Arial', 14), command=self.back_to_mode_selection, bg='red', fg='white')
    ...
    def update_timer(self):
        ...
        if remaining.total_seconds() > 0:
            ...
            seconds = seconds % 60
            self.timer_label.config(text=f"Time remaining: {days} days, {hours:02}:{minutes:02}:{seconds:02}")
            self.timer_id = self.root.after(1000, self.update_timer)
        else:
            self.timer_label.config(text="Event has started!")
    ...
    def play_video(self):
        video_root = tk.Tk()
        video_window = DemonSlayerTournament(video_root, "Tournament_video.mp4", self)
        video_root.state('zoomed')
        video_root.mainloop()
...
class DemonSlayerTournament:
    def __init__(self, root, video_path, parent_window):
        self.root = root
        self.video_path = video_path
        self.parent_window = parent_window
        self.playing = True

        self.root.bind("<Return>", self.on_video_end)

        self.label = tk.Label(self.root)
        self.label.pack(fill=tk.BOTH, expand=1)

        self.video_thread = threading.Thread(target=self.initialize_vlc)
        self.video_thread.start()

        self.root.protocol("WM_DELETE_WINDOW", self.on_closing)

    def check_video_end(self):
        if self.player.get_state() == vlc.State.Ended or not self.playing:
            self.on_video_end()
        else:
    ...
    def stop_game_audio(self):
        pygame.mixer.music.pause()

    def resume_game_audio(self):
...
class InstructionsWindow:
    def __init__(self, root):
        ...
        self.add_video_demo_placeholder()

        # Display the initial message
        self.show_message("Follow the instructions to learn how to play Futoshiki!")

    def add_video_demo_placeholder(self):
        self.video_placeholder = tk.Canvas(self.root, width=300, height=150, bg='black')
        self.video_placeholder.place(relx=0.7, rely=0.7)
        self.play_button = tk.Button(self.video_placeholder, text="Watch Demo", bg='gray', command=self.play_demo_video)
    ...
    def play_demo_video(self):
        ...
        self.video_window = Toplevel(self.root)
        self.video_window.geometry("640x480")
        self.video_window.title("Demo Video")

        # Override the close event
        self.video_window.protocol("WM_DELETE_WINDOW", self.close_demo_video)

        try:
            # VLC options
            vlc_options = ["--no-fullscreen", "--aout=waveout", "--vout=direct3d11"]

            self.instance = vlc.Instance(vlc_options)
            self.player = self.instance.media_player_new()
            self.player.set_hwnd(self.video_window.winfo_id())

            media = self.instance.media_new_path(r"demo_v.mp4")
            self.player.set_media(media)
            self.player.play()

        except Exception as e:
            print(f"Error initializing VLC: {e}")
            pygame.mixer.music.unpause()  # Resume the background music if VLC fails
    ...
    def close_demo_video(self):
        try:
            ...
            print(f"Error closing demo video: {e}")
